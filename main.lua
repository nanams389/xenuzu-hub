...
-- 設定用変数
_G.IndividualLoop = false

DestTab:AddToggle({
    Name = "Target: 地上爆速Flingループ",
    Default = false,
    Callback = function(Value)
        _G.IndividualLoop = Value
        if Value then
            task.spawn(function()
                local RunService = game:GetService("RunService")
                while _G.IndividualLoop do
                    local target = game.Players:FindFirstChild(_G.PlayerToLongGrab)
                    local char = game.Players.LocalPlayer.Character
                    ...
                    if target and target.Character and seat and seat.Parent then
                        ...
                        if targetHRP and remote then
                            -- [[ ステップ1: 地上で相手をハメるための密着TP ]]
                            -- 相手の目の前かつ、少し押し込む位置にTP
                            blobRoot.CFrame = targetHRP.CFrame * CFrame.new(0, 0, -1)

                            -- [[ ステップ2: 自分のキャラをフリーズから保護 ]]
                            for _, v in pairs(char:GetDescendants()) do
                                if v:IsA("BasePart") then v.Massless = true v.CanCollide = false end
                            end

                            -- [[ ステップ3: Fling誘発 & 高速ループ掴み ]]
                            -- Detectorを高速で振り回すようなパケットを送信
                            local arms = {"Left", "Right"}
                            for i = 1, 10 do -- 1サイクルで10連打
                                for _, side in ipairs(arms) do
                                    local det = blobman:FindFirstChild(side .. "Detector")
                                    local weld = det and (det:FindFirstChild(side .. "Weld") or det:FindFirstChildWhichIsA("Weld"))
                                    ...
                                    if det and weld then
                                        -- 物理をバグらせるための座標変更
                                        det.CFrame = targetHRP.CFrame * CFrame.Angles(math.rad(math.random(-180, 180)), 0, 0)
