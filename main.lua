...
-- タブ：Formation Hub
-- OrionLib:Init() の前に挿入
-- do...end でスコープ隔離してローカル変数上限エラーを回避
--==============================
do
...
-- 全状態を1テーブルに集約（ローカル変数節約が目的）
local FM = {
    enabled=false, partName="GlassBoxGray", mode="Robot",
    count=20, useAllFound=false, speed=1.0, smoothness=0.12,
    objs={}, time=0, conn=nil,
    rScale=1.2, rBobA=0.15, rBobS=1.0, rArmSw=25, rLegSw=20,
    sRadius=5, sRotSp=1.2, sLayers=3, sAltSpin=true,
    mScale=1.5, mClawF=30, mSpineA=0.8, mTailL=6,
    gScale=2.5, gBobA=0.2, gBobS=0.8, gArmSw=30, gLegSw=25,
}

local function findByName(name)
    local r={}
    for _,v in ipairs(workspace:GetDescendants()) do
        if (v:IsA("BasePart") or v:IsA("Model")) and v.Name==name then r[#r+1]=v end
    end
    return r
end

local function setupObj(obj)
    local base = obj:IsA("Model") and (obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")) or (obj:IsA("BasePart") and obj)
    if not base then return nil end
    if obj:IsA("Model") then
        for _,c in ipairs(obj:GetDescendants()) do
            if c:IsA("BasePart") then c.CanCollide=false; c.CanTouch=false; c.Anchored=false end
    ...
    for _,n in ipairs({"BodyPosition","BodyGyro"}) do
        local e=base:FindFirstChildOfClass(n); if e then e:Destroy() end
    end
    local BP=Instance.new("BodyPosition"); BP.P=25000; BP.D=800; BP.MaxForce=Vector3.new(1,1,1)*1e10; BP.Parent=base
    local BG=Instance.new("BodyGyro");    BG.P=25000; BG.D=800; BG.MaxTorque=Vector3.new(1,1,1)*1e10; BG.Parent=base
    return {BP=BP,BG=BG,Part=base}
end

local function releaseObj(o)
    if o.BP and o.BP.Parent then o.BP:Destroy() end
    if o.BG and o.BG.Parent then o.BG:Destroy() end
    if o.Part and o.Part.Parent then o.Part.Anchored=true end
end

local function rXZ(x,z,yr) return x*math.cos(yr)-z*math.sin(yr), x*math.sin(yr)+z*math.cos(yr) end

-- Robot
local function robotPos(i,t,cPos,yr)
    local s=FM.rScale; local b=math.sin(t*FM.rBobS)*FM.rBobA; local wc=math.sin(t*3)
    local aL=-math.rad(FM.rArmSw)*wc; local aR=math.rad(FM.rArmSw)*wc
    local lL= math.rad(FM.rLegSw)*wc; local lR=-math.rad(FM.rLegSw)*wc
    local p={
        {0,3.2*s+b,0},{0,2.5*s+b,0},{0,1.8*s+b,0},{0,1.2*s+b,0},{0,0.6*s+b,0},{0,0+b,0},
        {-1*s,1.9*s+b,0},{1*s,1.9*s+b,0},
        {-1.3*s,1.2*s+b+math.sin(aL)*0.8*s,math.cos(aL)*0.4*s-0.4*s},
        { 1.3*s,1.2*s+b+math.sin(aR)*0.8*s,math.cos(aR)*0.4*s-0.4*s},
        {-1.4*s,0.4*s+b+math.sin(aL)*1.4*s,math.cos(aL)*0.7*s-0.7*s},
        { 1.4*s,0.4*s+b+math.sin(aR)*1.4*s,math.cos(aR)*0.7*s-0.7*s},
        {-1.4*s,-0.3*s+b+math.sin(aL)*1.8*s,math.cos(aL)*1.0*s-1.0*s},
        { 1.4*s,-0.3*s+b+math.sin(aR)*1.8*s,math.cos(aR)*1.0*s-1.0*s},
        {-0.5*s,-0.6*s+math.sin(lL)*0.6*s,math.cos(lL)*0.5*s-0.5*s},
        { 0.5*s,-0.6*s+math.sin(lR)*0.6*s,math.cos(lR)*0.5*s-0.5*s},
        {-0.55*s,-1.5*s+math.sin(lL)*1.2*s,math.cos(lL)*1.0*s-1.0*s},
        { 0.55*s,-1.5*s+math.sin(lR)*1.2*s,math.cos(lR)*1.0*s-1.0*s},
    }
    local dx,dy,dz
    if i<=#p then local q=p[i];dx=q[1];dy=q[2];dz=q[3]
    else local e=i-#p;local a=(e-1)/math.max(1,FM.count-#p)*math.pi*2+t*FM.speed
        dx=math.cos(a)*0.6*s;dy=4*s+e*0.3*s+b;dz=math.sin(a)*0.6*s end
    local rx,rz=rXZ(dx,dz,yr); return cPos+Vector3.new(rx,dy,rz)
end

local function robotCF(i,pos,yr,t)
    local base=CFrame.new(pos)*CFrame.Angles(0,yr,0); local wc=math.sin(t*3)
    local aL=-math.rad(FM.rArmSw)*wc; local aR=math.rad(FM.rArmSw)*wc
    local lL= math.rad(FM.rLegSw)*wc; local lR=-math.rad(FM.rLegSw)*wc
    if i==9 or i==11 or i==13 then return base*CFrame.Angles(aL,0,0)
    elseif i==10 or i==12 or i==14 then return base*CFrame.Angles(aR,0,0)
    elseif i==15 or i==17 then return base*CFrame.Angles(lL,0,0)
    elseif i==16 or i==18 then return base*CFrame.Angles(lR,0,0) end
    return base
end

-- Sphere
local function spherePos(i,total,t,cPos,yr)
    local r=FM.sRadius
    local phi=math.acos(1-2*i/total); local theta=math.pi*(1+math.sqrt(5))*i
    local x=r*math.sin(phi)*math.cos(theta); local y=r*math.cos(phi); local z=r*math.sin(phi)*math.sin(theta)
    local layer=math.floor(i/(total/math.max(FM.sLayers,1)))
    local dir=(FM.sAltSpin and layer%2==0) and 1 or -1
    local sp=t*FM.sRotSp*dir
    local rx2=x*math.cos(sp)-z*math.sin(sp); local rz2=x*math.sin(sp)+z*math.cos(sp)
    local fx,fz=rXZ(rx2,rz2,yr); return cPos+Vector3.new(fx,y,fz)
end

-- Monster
local function monsterPos(i,total,t,cPos,yr)
    local s=FM.mScale; local ct=t*FM.speed
    local cN=4;local fN=2;local hN=2
    local spN=math.floor((total-cN-fN-hN)*0.4); local tlN=total-cN-fN-hN-spN
    local dx,dy,dz=0,0,0
    if i<=cN then
        local sd=(i<=2) and -1 or 1; local up=(i%2==1) and 0 or 1
        local ang=math.sin(ct)*0.3+math.rad(FM.mClawF)
        dx=sd*(1.8+up*0.6)*s+sd*math.cos(ang)*1.2*s
        dy=(0.5-up*0.8)*s+math.sin(ct+i)*0.2*s; dz=-(0.5+up*0.5)*s-math.sin(ang)*0.8*s
    elseif i<=cN+fN then
        local fi=i-cN; local sd=(fi==1) and -0.4 or 0.4
        dx=sd*s; dy=2.5*s+math.sin(ct*0.5)*0.1*s; dz=-1.0*s
    elseif i<=cN+fN+hN then
        local hi=i-cN-fN; local sd=(hi==1) and -0.5 or 0.5
        dx=sd*(0.4+(hi-1)*0.2)*s; dy=(3.8+(hi-1)*0.6)*s; dz=-0.2*s
    elseif i<=cN+fN+hN+spN then
        local si=i-cN-fN-hN; local pg=(si-1)/math.max(spN-1,1)
        dz=(-1+pg*2)*s; dy=(2.5-pg*1.5)*s+math.sin(ct*2+pg*math.pi*3)*FM.mSpineA*s
        dx=math.sin(ct+pg*1.5)*0.15*s
    else
        local ti=i-cN-fN-hN-spN; local pg=ti/math.max(tlN,1)
        dz=(0.8+pg*FM.mTailL*0.5)*s; dy=(-0.2-pg*1.5)*s+math.sin(ct*1.5+pg*math.pi*4)*(0.5+pg)*s
        dx=math.sin(ct*0.8+pg*math.pi*2)*(0.3+pg*0.4)*s
    end
    local rx,rz=rXZ(dx,dz,yr); return cPos+Vector3.new(rx,dy,rz)
end

-- Giant
local function giantPos(i,total,t,cPos,yr)
    local s=FM.gScale; local b=math.sin(t*FM.gBobS)*FM.gBobA; local wc=math.sin(t*2)
    local aL=-math.rad(FM.gArmSw)*wc; local aR=math.rad(FM.gArmSw)*wc
    local lL= math.rad(FM.gLegSw)*wc; local lR=-math.rad(FM.gLegSw)*wc
    local p={
        {0,4.5*s+b,0},{0,3.5*s+b,0},
        {0,2.5*s+b,0.9*s},{0,2.5*s+b,-0.9*s},{1.5*s,2.0*s+b,0},{-1.5*s,2.0*s+b,0},
        {0,1.5*s+b,0.9*s},{0,1.5*s+b,-0.9*s},{0,0.6*s+b,0.9*s},{0,0.6*s+b,-0.9*s},
        {-2.2*s,3.0*s+b,0},{2.2*s,3.0*s+b,0},
        {-2.5*s,2.2*s+b+math.sin(aL)*0.9*s,math.cos(aL)*0.5*s-0.5*s},
        { 2.5*s,2.2*s+b+math.sin(aR)*0.9*s,math.cos(aR)*0.5*s-0.5*s},
        {-2.7*s,1.2*s+b+math.sin(aL)*1.8*s,math.cos(aL)*1.0*s-1.0*s},
        { 2.7*s,1.2*s+b+math.sin(aR)*1.8*s,math.cos(aR)*1.0*s-1.0*s},
        {-2.7*s,0.2*s+b+math.sin(aL)*2.5*s,math.cos(aL)*1.4*s-1.4*s},
        { 2.7*s,0.2*s+b+math.sin(aR)*2.5*s,math.cos(aR)*1.4*s-1.4*s},
        {-0.9*s,-0.5*s+math.sin(lL)*0.8*s,math.cos(lL)*0.6*s-0.6*s},
        { 0.9*s,-0.5*s+math.sin(lR)*0.8*s,math.cos(lR)*0.6*s-0.6*s},
        {-1.0*s,-2.0*s+math.sin(lL)*1.5*s,math.cos(lL)*1.2*s-1.2*s},
        { 1.0*s,-2.0*s+math.sin(lR)*1.5*s,math.cos(lR)*1.2*s-1.2*s},
        {-1.0*s,-3.2*s+math.sin(lL)*1.8*s,math.cos(lL)*1.5*s-1.5*s},
        { 1.0*s,-3.2*s+math.sin(lR)*1.8*s,math.cos(lR)*1.5*s-1.5*s},
    }
    local dx,dy,dz
    if i<=#p then local q=p[i];dx=q[1];dy=q[2];dz=q[3]
    else local e=i-#p;local a=(e-1)/math.max(1,total-#p)*math.pi*2+t*FM.speed
        dx=math.cos(a)*0.8*s;dy=5.5*s+e*0.4*s+b;dz=math.sin(a)*0.8*s end
    local rx,rz=rXZ(dx,dz,yr); return cPos+Vector3.new(rx,dy,rz)
end

local function giantCF(i,pos,yr,t)
    local base=CFrame.new(pos)*CFrame.Angles(0,yr,0); local wc=math.sin(t*2)
    local aL=-math.rad(FM.gArmSw)*wc; local aR=math.rad(FM.gArmSw)*wc
    local lL= math.rad(FM.gLegSw)*wc; local lR=-math.rad(FM.gLegSw)*wc
    if i==13 or i==15 or i==17 then return base*CFrame.Angles(aL,0,0)
    elseif i==14 or i==16 or i==18 then return base*CFrame.Angles(aR,0,0)
    elseif i==19 or i==21 or i==23 then return base*CFrame.Angles(lL,0,0)
    elseif i==20 or i==22 or i==24 then return base*CFrame.Angles(lR,0,0) end
    return base
end

local function fmStop()
    if FM.conn then FM.conn:Disconnect(); FM.conn=nil end
    for _,o in ipairs(FM.objs) do releaseObj(o) end; FM.objs={}
end

local function fmStart()
    fmStop()
    local found=findByName(FM.partName)
    if #found==0 then OrionLib:MakeNotification({Name="エラー",Content="'"..FM.partName.."' なし",Time=3}); return end
    local uc=FM.useAllFound and #found or math.min(FM.count,#found)
    for i=1,uc do local o=setupObj(found[i]); if o then FM.objs[#FM.objs+1]=o end end
    OrionLib:MakeNotification({Name="Formation",Content=FM.mode.."/"..#FM.objs.."個",Time=2})
    FM.time=0
    FM.conn=RS.RenderStepped:Connect(function(dt)
        if not FM.enabled then return end
        local char=LP.Character; if not char then return end
        local hrp=char:FindFirstChild("HumanoidRootPart"); if not hrp then return end
        FM.time+=dt
        local t=FM.time; local cPos=hrp.Position; local cCF=hrp.CFrame
        local _,yr,_=cCF:ToOrientation(); local sm=FM.smoothness; local total=#FM.objs
        if FM.mode=="Robot" then
            for i,o in ipairs(FM.objs) do
                if o.BP and o.BP.Parent then
                    local pos=robotPos(i,t,cPos,yr)
                    o.BP.Position=o.BP.Position+(pos-o.BP.Position)*sm
                    if o.BG and o.BG.Parent then o.BG.CFrame=o.BG.CFrame:Lerp(robotCF(i,pos,yr,t),sm) end
                end
            end
        elseif FM.mode=="Sphere" then
            for i,o in ipairs(FM.objs) do
                if o.BP and o.BP.Parent then
                    local pos=spherePos(i,total,t,cPos,yr)
                    o.BP.Position=o.BP.Position+(pos-o.BP.Position)*sm
                    if o.BG and o.BG.Parent then o.BG.CFrame=o.BG.CFrame:Lerp(CFrame.lookAt(pos,cPos),0.15) end
                end
            end
        elseif FM.mode=="Monster" then
            for i,o in ipairs(FM.objs) do
                if o.BP and o.BP.Parent then
                    local pos=monsterPos(i,total,t,cPos,yr)
                    o.BP.Position=o.BP.Position+(pos-o.BP.Position)*sm
                    if o.BG and o.BG.Parent then o.BG.CFrame=o.BG.CFrame:Lerp(CFrame.new(pos)*CFrame.Angles(0,yr,0),sm) end
                end
            end
        elseif FM.mode=="Giant" then
            for i,o in ipairs(FM.objs) do
                if o.BP and o.BP.Parent then
                    local pos=giantPos(i,total,t,cPos,yr)
                    o.BP.Position=o.BP.Position+(pos-o.BP.Position)*sm
                    if o.BG and o.BG.Parent then o.BG.CFrame=o.BG.CFrame:Lerp(giantCF(i,pos,yr,t),sm) end
                end
            end
        end
    end)
...
-- UI
local FmTab=Window:MakeTab({Name="Formation",Icon="rbxassetid://4483345998",PremiumOnly=false})
FmTab:AddSection({Name="⚙️ Formation 制御"})
FmTab:AddToggle({Name="有効化",Default=false,Flag="FmEnabled",Callback=function(v)
    FM.enabled=v; if v then fmStart() else fmStop(); OrionLib:MakeNotification({Name="停止",Content="停止",Time=2}) end end})
FmTab:AddTextbox({Name="対象Part名",Default="GlassBoxGray",TextDisappear=false,Callback=function(v) FM.partName=v end})
FmTab:AddDropdown({Name="モード",Default="Robot",Options={"Robot","Sphere","Monster","Giant"},Callback=function(v)
    FM.mode=v; if FM.enabled then fmStart() end end})
FmTab:AddToggle({Name="全Part使用",Default=false,Flag="FmUseAll",Callback=function(v) FM.useAllFound=v end})
FmTab:AddSlider({Name="使用数",Min=1,Max=60,Default=20,Increment=1,ValueName="個",Callback=function(v) FM.count=v end})
FmTab:AddSlider({Name="アニメ速度",Min=0,Max=5,Default=1.0,Increment=0.1,ValueName="",Callback=function(v) FM.speed=v end})
FmTab:AddSlider({Name="滑らかさ",Min=0.01,Max=1,Default=0.12,Increment=0.01,ValueName="",Callback=function(v) FM.smoothness=v end})
FmTab:AddButton({Name="再検索 & 再起動",Callback=function()
    if FM.enabled then fmStart() else local f=findByName(FM.partName)
        OrionLib:MakeNotification({Name="検索",Content=#f.."個発見",Time=3}) end end})

FmTab:AddSection({Name="🤖 Robot（18部位人体型）"})
FmTab:AddSlider({Name="Robot: スケール",Min=0.3,Max=4,Default=1.2,Increment=0.1,ValueName="x",Callback=function(v) FM.rScale=v end})
FmTab:AddSlider({Name="Robot: 呼吸の揺れ",Min=0,Max=1,Default=0.15,Increment=0.05,ValueName="",Callback=function(v) FM.rBobA=v end})
FmTab:AddSlider({Name="Robot: 呼吸の速度",Min=0,Max=5,Default=1.0,Increment=0.1,ValueName="",Callback=function(v) FM.rBobS=v end})
FmTab:AddSlider({Name="Robot: 腕の振り角度",Min=0,Max=90,Default=25,Increment=1,ValueName="°",Callback=function(v) FM.rArmSw=v end})
FmTab:AddSlider({Name="Robot: 脚の振り角度",Min=0,Max=60,Default=20,Increment=1,ValueName="°",Callback=function(v) FM.rLegSw=v end})

FmTab:AddSection({Name="🔮 Sphere（球状に全周を囲む）"})
FmTab:AddSlider({Name="Sphere: 半径",Min=1,Max=20,Default=5,Increment=0.5,ValueName="",Callback=function(v) FM.sRadius=v end})
FmTab:AddSlider({Name="Sphere: 回転速度",Min=-5,Max=5,Default=1.2,Increment=0.1,ValueName="",Callback=function(v) FM.sRotSp=v end})
FmTab:AddSlider({Name="Sphere: 層数",Min=1,Max=5,Default=3,Increment=1,ValueName="層",Callback=function(v) FM.sLayers=v end})
FmTab:AddToggle({Name="Sphere: 層ごと逆回転",Default=true,Flag="FmSphAlt",Callback=function(v) FM.sAltSpin=v end})

FmTab:AddSection({Name="👹 Monster（化け物型）"})
FmTab:AddSlider({Name="Monster: スケール",Min=0.3,Max=4,Default=1.5,Increment=0.1,ValueName="x",Callback=function(v) FM.mScale=v end})
FmTab:AddSlider({Name="Monster: 爪の広がり",Min=5,Max=80,Default=30,Increment=1,ValueName="°",Callback=function(v) FM.mClawF=v end})
FmTab:AddSlider({Name="Monster: 背びれ波打ち",Min=0,Max=3,Default=0.8,Increment=0.1,ValueName="",Callback=function(v) FM.mSpineA=v end})
FmTab:AddSlider({Name="Monster: 尾の長さ",Min=1,Max=15,Default=6,Increment=0.5,ValueName="",Callback=function(v) FM.mTailL=v end})

FmTab:AddSection({Name="🏔️ Giant（自分が胴体の中に入る巨人）"})
FmTab:AddSlider({Name="Giant: スケール",Min=0.5,Max=6,Default=2.5,Increment=0.1,ValueName="x",Callback=function(v) FM.gScale=v end})
FmTab:AddSlider({Name="Giant: 呼吸の揺れ",Min=0,Max=1,Default=0.2,Increment=0.05,ValueName="",Callback=function(v) FM.gBobA=v end})
FmTab:AddSlider({Name="Giant: 呼吸の速度",Min=0,Max=5,Default=0.8,Increment=0.1,ValueName="",Callback=function(v) FM.gBobS=v end})
FmTab:AddSlider({Name="Giant: 腕の振り角度",Min=0,Max=90,Default=30,Increment=1,ValueName="°",Callback=function(v) FM.gArmSw=v end})
FmTab:AddSlider({Name="Giant: 脚の振り角度",Min=0,Max=60,Default=25,Increment=1,ValueName="°",Callback=function(v) FM.gLegSw=v end})
