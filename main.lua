...
-- ============================================================
-- Anti Loop (Auto Plot Cycle) - プロット間移動版
-- ============================================================

local antiLoopActive = false
local currentTargetIndex = 1 -- 現在狙っているプロット番号

invulnerabilitySection:AddToggle({
    Name = "Enable Anti-Loop (Plot Cycler)",
    Default = false,
    Callback = function(Value)
                ...
                while antiLoopActive do
                    ...
                    pcall(function()
                        ...
                        if hrp and targetCFrame then
                            ...
                            -- 【判定】死んだか、または現在のプロットから外に出たか
                            if (hum and hum.Health <= 0) or dist > 70 then
                                
                                -- 外に出た場合のみ、次のプロット番号へ更新（1→2→3→4→5→1...）
                                if hum and hum.Health > 0 then
                                    currentTargetIndex = currentTargetIndex + 1
                                    if currentTargetIndex > 5 then
                                ...
                                if targetCFrame then
                                    ...
                                    OrionLib:MakeNotification({
                                        Name = "Anti-Loop",
                                        Content = "Plot " .. currentTargetIndex .. " へ移動しました",
                                        Time = 1
                                    })
                                end
                            end
...
--==================================================
-- [F] YouDecoy (BallMagicLight構造版) 設定
-- ※ ballConfig と同じ構造で独立して動作
-- ※ Wings & Orbit タブの BallMagicLight セクションの後ろに挿入
--==================================================

local ydBallConfig = {
    mode = "Wings",
    enabled = false,
    targetPartName = "YouDecoy",  -- ← BallMagicLight から変更
    count = 10,
    useAllFound = true,
    speed = 2.5,
    height = 1,
...
local function ydBallGetOrbitPos(index, total, t, charPos, charCF)
    ...
    if shape == "Circle" then
        ...
        x = math.cos(a) * r; z = math.sin(a) * r; y = ydBallConfig.height
    elseif shape == "Figure8" then
        local a = t * ydBallConfig.speed + angle
        x = math.sin(a) * r; z = math.sin(a * 2) * r * 0.5; y = ydBallConfig.height
    elseif shape == "Wave" then
        local a = angle + t * ydBallConfig.speed
        x = math.cos(a) * r; z = math.sin(a) * r
        y = ydBallConfig.height + math.sin(a * ydBallConfig.orbitWaveFrequency + t * ydBallConfig.speed) * ydBallConfig.orbitWaveAmplitude
    elseif shape == "Helix" then
        local a = angle + t * ydBallConfig.speed
        x = math.cos(a) * r; z = math.sin(a) * r
        local prog = ((t * ydBallConfig.speed * 0.3) % 1)
        y = ydBallConfig.height + (prog * ydBallConfig.orbitHelixHeight) - (ydBallConfig.orbitHelixHeight / 2)
        y = y + (index / total) * ydBallConfig.orbitHelixHeight
    elseif shape == "Star" then
        local pts = ydBallConfig.orbitStarPoints
        local innerR = ydBallConfig.orbitStarInnerRadius * ydBallConfig.orbitScale
        local a = angle + t * ydBallConfig.speed
        local pA = (math.pi * 2) / pts
        local blend = (a % pA) / pA
        local dist = innerR + (r - innerR) * math.abs(math.sin(blend * math.pi))
        x = math.cos(a) * dist; z = math.sin(a) * dist; y = ydBallConfig.height
    elseif shape == "Sphere" then
        local theta = (index / total) * math.pi + t * ydBallConfig.speed * 0.3
        x = r * math.sin(theta) * math.cos(angle + t * ydBallConfig.speed)
        z = r * math.sin(theta) * math.sin(angle + t * ydBallConfig.speed)
        y = ydBallConfig.height + r * math.cos(theta)
    elseif shape == "DNA" then
        local a = angle + t * ydBallConfig.speed
        local strand = (index % 2 == 0) and 1 or -1
        x = math.cos(a + strand * math.pi) * r
        z = math.sin(a + strand * math.pi) * r * 0.3
        y = ydBallConfig.height + (a / (math.pi * 2)) * ydBallConfig.orbitHelixHeight * 0.5
    end
...
local function ydBallStart()
    ...
    if #found == 0 then
        OrionLib:MakeNotification({ Name = "エラー", Content = "'" .. ydBallConfig.targetPartName .. "' が見つかりません", Time = 3 })
        return
    end
    local useCount = ydBallConfig.useAllFound and #found or math.min(ydBallConfig.count, #found)
    for i = 1, useCount do
        local obj = _setupObj(found[i])
        if obj then table.insert(ydBallParts, obj) end
    end
    if ydBallConfig.mode == "Wings" then ydBallRowPoints = ydBallCreateWingsPoints(#ydBallParts) end
    OrionLib:MakeNotification({ Name = "YouDecoy 起動", Content = "モード: " .. ydBallConfig.mode .. " / 数: " .. #ydBallParts, Time = 2 })
    ydBallTime = 0
    ydBallConnection = RunService.RenderStepped:Connect(function(dt)
        ...
        if ydBallConfig.mode == "Wings" then
            ...
            for i, obj in ipairs(ydBallParts) do
                if obj.BP and obj.BP.Parent and obj.BG and obj.BG.Parent then
                    local pt = ydBallRowPoints[i]
                    if not pt then continue end
                    local dist = math.abs(pt.baseOffsetX)
                    local amp = ydBallConfig.wingsAmplitude + dist * ydBallConfig.wingsDistMultiplier
                    local wave = math.sin(ydBallTime + pt.index * ydBallConfig.wingsPhaseOffset)
                    local hOff = pt.baseOffsetX
                    if pt.baseOffsetX ~= 0 then
                        local sign = pt.baseOffsetX > 0 and 1 or -1
                        hOff = pt.baseOffsetX - sign * math.abs(pt.baseOffsetX) * wave * ydBallConfig.wingsHorizontalAmount
                    end
                    local targetPos = basePos + charCF.RightVector * hOff + Vector3.new(0, wave * amp, 0)
                    obj.BP.Position = obj.BP.Position + (targetPos - obj.BP.Position) * ydBallConfig.smoothness
                    local cf = CFrame.new(targetPos)
                    local _, pYR, _ = charCF:ToOrientation()
                    cf = cf * CFrame.Angles(0, pYR + math.rad(ydBallConfig.wingsYRot), 0)
                    cf = cf * CFrame.Angles(math.rad(ydBallConfig.wingsXRot), 0, 0)
                    cf = cf * CFrame.Angles(0, 0, math.rad(ydBallConfig.wingsZRot))
        ...
        elseif ydBallConfig.mode == "Orbit" then
            for i, obj in ipairs(ydBallParts) do
                if obj.BP and obj.BP.Parent and obj.BG and obj.BG.Parent then
                    ...
                    if ydBallConfig.orbitFaceCenter then
                        obj.BG.CFrame = obj.BG.CFrame:Lerp(CFrame.lookAt(pos, hrp.Position), 0.2)
                    end
                end
            end
...
-- ============================================
-- セクション: YouDecoy (BallMagicLight構造)
-- ============================================
...
WingsTab:AddToggle({
    ...
    Flag = "YDBallEnabled",
    Callback = function(v)
        ydBallConfig.enabled = v
        if v then ydBallStart()
        else ydBallStop(); OrionLib:MakeNotification({ Name = "YD停止", Content = "停止しました", Time = 2 }) end
    end,
})

WingsTab:AddDropdown({ Name = "YD: モード", Default = "Wings", Options = { "Wings", "Orbit" },
    Callback = function(v) ydBallConfig.mode = v; if ydBallConfig.enabled then ydBallStart() end end })

WingsTab:AddTextbox({ Name = "YD: 対象Part名", Default = "YouDecoy", TextDisappear = false,
    Callback = function(v) ydBallConfig.targetPartName = v end })

WingsTab:AddToggle({ Name = "YD: 全Part使用", Default = true, Flag = "YDBallUseAll",
    Callback = function(v) ydBallConfig.useAllFound = v end })
WingsTab:AddSlider({ Name = "YD: 使用数", Min = 1, Max = 60, Default = 10, Increment = 1, ValueName = "体",
    Callback = function(v) ydBallConfig.count = v end })
WingsTab:AddSlider({ Name = "YD: 速度", Min = -10, Max = 10, Default = 2.5, Increment = 0.1, ValueName = "",
    Callback = function(v) ydBallConfig.speed = v end })
WingsTab:AddSlider({ Name = "YD: 高さ", Min = -5, Max = 15, Default = 1, Increment = 0.5, ValueName = "",
    Callback = function(v) ydBallConfig.height = v end })
WingsTab:AddSlider({ Name = "YD: 滑らかさ", Min = 0.01, Max = 1, Default = 0.15, Increment = 0.01, ValueName = "",
    Callback = function(v) ydBallConfig.smoothness = v end })

-- Wings専用
WingsTab:AddSlider({ Name = "YD Wings: 間隔", Min = 0.3, Max = 5, Default = 1.2, Increment = 0.1, ValueName = "",
    Callback = function(v) ydBallConfig.wingsSpacing = v; if ydBallConfig.enabled and ydBallConfig.mode == "Wings" then ydBallRowPoints = ydBallCreateWingsPoints(#ydBallParts) end end })
WingsTab:AddSlider({ Name = "YD Wings: 振幅", Min = 0, Max = 10, Default = 2.5, Increment = 0.1, ValueName = "",
    Callback = function(v) ydBallConfig.wingsAmplitude = v end })
WingsTab:AddSlider({ Name = "YD Wings: 位相差", Min = 0, Max = 2, Default = 0.3, Increment = 0.05, ValueName = "",
    Callback = function(v) ydBallConfig.wingsPhaseOffset = v end })
WingsTab:AddSlider({ Name = "YD Wings: 距離振幅増加", Min = 0, Max = 2, Default = 0.4, Increment = 0.05, ValueName = "",
    Callback = function(v) ydBallConfig.wingsDistMultiplier = v end })
WingsTab:AddSlider({ Name = "YD Wings: 内側寄り", Min = 0, Max = 2, Default = 0.5, Increment = 0.05, ValueName = "",
    Callback = function(v) ydBallConfig.wingsHorizontalAmount = v end })
WingsTab:AddSlider({ Name = "YD Wings: 前方オフセット", Min = 0, Max = 15, Default = 3, Increment = 0.5, ValueName = "",
    Callback = function(v) ydBallConfig.wingsForwardOffset = v end })
WingsTab:AddSlider({ Name = "YD Wings: X軸回転", Min = -180, Max = 180, Default = -45, Increment = 1, ValueName = "°",
    Callback = function(v) ydBallConfig.wingsXRot = v end })
WingsTab:AddSlider({ Name = "YD Wings: Y軸回転", Min = -180, Max = 180, Default = 0, Increment = 1, ValueName = "°",
    Callback = function(v) ydBallConfig.wingsYRot = v end })
WingsTab:AddSlider({ Name = "YD Wings: Z軸回転", Min = -180, Max = 180, Default = 90, Increment = 1, ValueName = "°",
    Callback = function(v) ydBallConfig.wingsZRot = v end })
WingsTab:AddButton({ Name = "YD Wings: 回転リセット", Callback = function()
    ydBallConfig.wingsXRot = -45; ydBallConfig.wingsYRot = 0; ydBallConfig.wingsZRot = 90
    OrionLib:MakeNotification({ Name = "リセット", Content = "Wings回転リセット", Time = 2 })
end })

-- Orbit専用
WingsTab:AddDropdown({ Name = "YD Orbit: 形状", Default = "Circle",
    Options = { "Circle", "Figure8", "Wave", "Helix", "Star", "Sphere", "DNA" },
    Callback = function(v) ydBallConfig.orbitShape = v end })
WingsTab:AddSlider({ Name = "YD Orbit: 半径", Min = 1, Max = 30, Default = 5, Increment = 0.5, ValueName = "",
    Callback = function(v) ydBallConfig.orbitRadius = v end })
WingsTab:AddSlider({ Name = "YD Orbit: スケール", Min = 0.1, Max = 5, Default = 1, Increment = 0.1, ValueName = "x",
    Callback = function(v) ydBallConfig.orbitScale = v end })
WingsTab:AddSlider({ Name = "YD Orbit: Y軸オフセット", Min = -180, Max = 180, Default = 0, Increment = 1, ValueName = "°",
    Callback = function(v) ydBallConfig.orbitYRotOffset = v end })
WingsTab:AddSlider({ Name = "YD Orbit: 波振幅", Min = 0, Max = 10, Default = 2, Increment = 0.1, ValueName = "",
    Callback = function(v) ydBallConfig.orbitWaveAmplitude = v end })
WingsTab:AddSlider({ Name = "YD Orbit: 波周波数", Min = 1, Max = 10, Default = 2, Increment = 0.5, ValueName = "",
    Callback = function(v) ydBallConfig.orbitWaveFrequency = v end })
WingsTab:AddSlider({ Name = "YD Orbit: 螺旋高さ", Min = 1, Max = 20, Default = 5, Increment = 0.5, ValueName = "",
    Callback = function(v) ydBallConfig.orbitHelixHeight = v end })
WingsTab:AddSlider({ Name = "YD Orbit: 星頂点数", Min = 3, Max = 12, Default = 5, Increment = 1, ValueName = "点",
    Callback = function(v) ydBallConfig.orbitStarPoints = v end })
WingsTab:AddSlider({ Name = "YD Orbit: 星内半径", Min = 0.5, Max = 10, Default = 2, Increment = 0.5, ValueName = "",
    Callback = function(v) ydBallConfig.orbitStarInnerRadius = v end })
WingsTab:AddToggle({ Name = "YD Orbit: 中心を向く", Default = false, Flag = "YDBallFaceCenter",
    Callback = function(v) ydBallConfig.orbitFaceCenter = v end })
WingsTab:AddButton({ Name = "YD: 再検索 & 再起動", Callback = function()
    if ydBallConfig.enabled then ydBallStart()
    else
        local f = _findByName(ydBallConfig.targetPartName)
        OrionLib:MakeNotification({ Name = "検索", Content = #f .. "体発見", Time = 3 })
    end
end })
