                    ...
                    for _, player in pairs(game.Players:GetPlayers()) do
                        ...
                        pcall(function()
                            ...
                            if targetHRP and targetHum and targetHum.Health > 0 then
                                -- 1. 所有権奪取 (これが無いと奈落に落ちない)
                                local rs = game:GetService("ReplicatedStorage")
                                if rs:FindFirstChild("GrabEvents") and rs.GrabEvents:FindFirstChild("CreateGrabLine") then
                                    rs.GrabEvents.CreateGrabLine:FireServer(targetHRP)
                                end

                                -- 2. Noclip効果 (衝突判定を消して地面を抜ける)
                ...
                while _G.KickAll do
                    ...
                    for _, player in pairs(game.Players:GetPlayers()) do
                        ...
                        pcall(function()
                            ...
                            if targetHRP then
                                -- 1. 所有権を奪い、座標の強制権を得る
                                game:GetService("ReplicatedStorage").GrabEvents.CreateGrabLine:FireServer(targetHRP)
                                
                                -- 2. 座標を「マップ外の奈落」へ完全に飛ばす
                                -- これによりアンチチートのKickを誘発させる
                                targetHRP.CFrame = CFrame.new(0, -1000, 0)
                                targetHRP.Velocity = Vector3.new(0, -1000, 0)

                                -- 3. サーバーにあがき信号を送って位置を確定
